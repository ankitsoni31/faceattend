...
//         axios.get('https://faceattend-backend-kbrz.onrender.com/api/attendance/all', { headers: { Authorization: `Bearer ${token}` } }),
//         axios.get('https://faceattend-backend-kbrz.onrender.com/api/auth/users', { headers: { Authorization: `Bearer ${token}` } }),
//         axios.get('https://faceattend-backend-kbrz.onrender.com/api/subjects'),
//       ])
...
//     try {
//       await axios.post('https://faceattend-backend-kbrz.onrender.com/api/subjects/add', subjectForm, {
...
//     try {
//       await axios.delete(`https://faceattend-backend-kbrz.onrender.com/api/subjects/${id}`, {
//         headers: { Authorization: `Bearer ${token}` }
//       })
//       fetchData()
//     } catch {
...
//     try {
//       await axios.delete(
//         `https://faceattend-backend-kbrz.onrender.com/api/auth/delete-student/${deleteModal.student._id}`,
//         { headers: { Authorization: `Bearer ${token}` } }
//       )
//       closeDeleteModal()
//       fetchData()
...
const AdminPanel = () => {
  ...
  const [subjectForm, setSubjectForm] = useState({ name: '', code: '', time: '', department: '', year: '', batch: '', startTime: '', endTime: '' })
  const [subjectError, setSubjectError] = useState('')
  const [subjectLoading, setSubjectLoading] = useState(false)
  ...
  const handleAddSubject = async (e) => {
    ...
    try {
      ...
      setSubjectForm({ name: '', code: '', time: '', department: '', year: '', batch: '', startTime: '', endTime: '' })
      fetchData()
    } catch (err) {
      setSubjectError(err.response?.data?.message || 'Failed to add subject')
    } finally {
  ...
  // Records filtered by search + month + subject
  const filtered = records.filter(r => {
    const matchesSearch =
      r.student?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      r.student?.rollNumber?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  // Unique subject names for records filter dropdown
  const uniqueSubjects = [...new Set(records.map(r => r.subject).filter(Boolean))]

  // Month options derived from records
  const uniqueMonths = [...new Set(records.map(r => r.date?.slice(0, 7)).filter(Boolean))].sort().reverse()
            ...
            <div className="bg-[#0A1221] border border-amber-500/10 rounded-2xl p-4 sm:p-6">
              ...
              <form onSubmit={handleAddSubject} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-slate-400 mb-2 font-medium uppercase tracking-wider">
                      Start Time <span className="text-slate-600 normal-case">(Optional)</span>
                    </label>
                    <input
                      type="time"
                      value={subjectForm.startTime}
                      onChange={e => setSubjectForm({ ...subjectForm, startTime: e.target.value })}
                      className="w-full px-4 py-3 text-sm bg-[#04080F] border border-white/10 rounded-xl text-white outline-none focus:border-amber-500/50 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-400 mb-2 font-medium uppercase tracking-wider">
                      End Time <span className="text-slate-600 normal-case">(Optional)</span>
                    </label>
                    <input
                      type="time"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-slate-400 mb-2 font-medium uppercase tracking-wider">
                      Year <span className="text-slate-600 normal-case">(Optional)</span>
                    </label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs text-slate-400 mb-2 font-medium uppercase tracking-wider">
                      Batch <span className="text-slate-600 normal-case">(Optional)</span>
                    </label>
                    <select
                      value={subjectForm.batch}
                      onChange={e => setSubjectForm({ ...subjectForm, batch: e.target.value })}
                      className="w-full px-4 py-3 text-sm bg-[#04080F] border border-white/10 rounded-xl text-white outline-none focus:border-amber-500/50 transition-colors"
                    >
                    ...
                    <div key={i} className="flex items-center justify-between gap-3 p-4 bg-[#04080F] rounded-xl border border-white/5">
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {s.department && (
                            <span className="text-xs text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-md">{s.department}</span>
                          )}
                          {s.year && (
                            <span className="text-xs text-purple-400 bg-purple-400/10 px-2 py-0.5 rounded-md">{s.year}</span>
                          )}
                          {s.batch && (
                            <span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-md">Batch {s.batch}</span>
                          )}
                        </div>
                      </div>
                      <button
            ...
            <div className="flex flex-col gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                <select
                  ...
                  className="px-4 py-2.5 text-sm bg-[#04080F] border border-white/10 rounded-xl text-white outline-none focus:border-cyan-500/50 transition-colors"
                >
                  <option value="">All Departments</option>
                  <option value="BSc IT">BSc IT</option>
                  <option value="BCom">BCom</option>
                ...
                {(departmentFilter || yearFilter || batchFilter) && (
                  <button
                    onClick={() => { setDepartmentFilter(''); setYearFilter(''); setBatchFilter('') }}
                    className="px-3 py-2.5 text-xs text-slate-400 border border-white/10 rounded-xl hover:text-white hover:border-white/25 transition-all cursor-pointer"
                  >
                    ✕ Clear
                  </button>
                )}
              ...
              <>
                ...
                <div className="hidden md:block overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        {['Name', 'Roll No', 'Department', 'Year', 'Batch', 'Email', 'Face', 'Classes', 'Attendance %', 'Action'].map(h => (
                          <th key={h} className="text-left text-xs text-slate-400 uppercase tracking-wider pb-3 pr-4 font-medium">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {students
                        .filter(s => {
                          ...
                          return matchesSearch && matchesDepartment && matchesYear && matchesBatch
                        })
                        .map((s, i) => {
                          const stuRecs = records.filter(r => r.student?._id === s._id || r.student === s._id)
                          const pct = stuRecs.length > 0
                            ? Math.round((stuRecs.filter(r => r.status === 'present').length / stuRecs.length) * 100)
                            : 0
                          const isDefaulter = pct < 75 && stuRecs.length > 0
                          return (
                            <tr key={i} className="hover:bg-white/5 transition-colors">
                              ...
                              <td className="py-3 pr-4 text-slate-400">{s.rollNumber}</td>
                              <td className="py-3 pr-4 text-slate-400">{s.department || '—'}</td>
                              <td className="py-3 pr-4 text-slate-400">{s.year || '—'}</td>
                              ...
                              <td className="py-3 pr-4">
                                <div className="flex items-center gap-1.5">
                                  <span className="font-semibold" style={{ color: getStatusColor(pct) }}>{pct}%</span>
                                  {isDefaulter && <span className="text-xs text-amber-400">⚠️</span>}
                                </div>
                              </td>
                              <td className="py-3">
                                <button
                        ...
                        <div key={i} className="bg-[#04080F] border border-white/5 rounded-xl p-4">
                          ...
                          <div className="grid grid-cols-2 gap-y-2.5 gap-x-3 mb-3">
                            ...
                            <div>
                              ...
                              <p className="text-slate-300 text-xs font-medium">{s.department || '—'}</p>
                            </div>
                            <div>
                              <p className="text-slate-500 text-xs mb-0.5">Year / Batch</p>
                              <p className="text-slate-300 text-xs font-medium">{s.year || '—'} / {s.batch || '—'}</p>
                            </div>
                            <div>
                              <p className="text-slate-500 text-xs mb-0.5">Classes</p>
                              <p className="text-slate-300 text-xs font-medium">{stuRecs.length}</p>
            ...
            <div className="flex flex-col gap-3 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(monthFilter || subjectFilter) && (
                  <button
                    onClick={() => { setMonthFilter(''); setSubjectFilter('') }}
                    className="px-3 py-2.5 text-xs text-slate-400 border border-white/10 rounded-xl hover:text-white hover:border-white/25 transition-all cursor-pointer"
                  >
                    ✕ Clear
                  </button>
